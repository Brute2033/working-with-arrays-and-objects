...
let evenNumbers = mixedNumbers.filter((number, index, array) => number % 2 ===0) // = mixedNumbers.filter((number, index, array)=> number % 2 === 0)
...
let myStrongest = monstersInYourPocket.filter((monsters, index, array)=> monsters.CP > 200) // = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
*/
// function orderTotals(element, index, orders){
//   orders.map(orders =>(orders.price * (orders.tax + 1)))
//   return orders
    
// }
// console.log(orderTotals(orders))
  //   return orders.map(orders.price * (orders.tax + 1))
// orderTotals(orders)
// function orderTotals(orders){
//   return (orders.price * (order.tax + 1))
// }
// orders.map(orderTotals)
...
*/
let bob = purchases.filter(purchases => purchases.owner === "Bob")

let bobsTotal = bob.reduce((acc , {price}) => acc + price, 0)